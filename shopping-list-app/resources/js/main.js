var data = (localStorage.getItem('shopList')) ? JSON.parse(localStorage.getItem('shopList')):{
    shop: [],
    purchased: []
  };
  ...
  var purchaseSVG = '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 22 22" style="enable-background:new 0 0 22 22;" xml:space="preserve"><rect y="0" class="noFill" width="22" height="22"/><g><path class="fill" d="M9.7,14.4L9.7,14.4c-0.2,0-0.4-0.1-0.5-0.2l-2.7-2.7c-0.3-0.3-0.3-0.8,0-1.1s0.8-0.3,1.1,0l2.1,2.1l4.8-4.8c0.3-0.3,0.8-0.3,1.1,0s0.3,0.8,0,1.1l-5.3,5.3C10.1,14.3,9.9,14.4,9.7,14.4z"/></g></svg>';
  ...
  function rendershopList() {
    ...
    for (var i = 0; i < data.shop.length; i++) {
      var value = data.shop[i];
      addItemshopM(value);
    }
  
    for (var j = 0; j < data.purchased.length; j++) {
      var value = data.purchased[j];
  ...
  function purchaseItem() {
    ...
    // Check if the item should be added to the purchased list or to re-added to the shop list
    var target = (id === 'shop') ? document.getElementById('purchased'):document.getElementById('shop');
  ...
  function addItemshopM(text, purchased) {
    var list = (purchased) ? document.getElementById('purchased'):document.getElementById('shop');
    ...
    purchase.addEventListener('touchstart', purchaseItem);
